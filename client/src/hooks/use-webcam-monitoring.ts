...
export function useWebcamMonitoring({
  ...
  const [frameCounts, setFrameCounts] = useState<Map<number, number>>(new Map());
    ...
    eventSource.onmessage = (event) => {
      try {
        ...
        if ((frame as any).dataUrl && (frame as any).studentId) {
          ...
          setFrameCounts(prev => {
            const newMap = new Map(prev);
            const currentCount = newMap.get(frame.studentId) ?? 0;
            newMap.set(frame.studentId, currentCount + 1);
            return newMap;
          });
        }
      } catch (error) {
